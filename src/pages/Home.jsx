import { Container, Row, Col } from 'react-bootstrap';
import { Link } from 'react-router-dom';
import { motion } from 'framer-motion';
import heroBg from '../assets/hero.png';

const Home = () => {
  return (
    <div>
      {/* Hero Section */}
      <section className="hero-sdn" style={{ position: 'relative', minHeight: '85vh', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
        <img src={heroBg} alt="Lathe Machine Operation" className="hero-bg-sdn" style={{ width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', top: 0, left: 0, zIndex: 0 }} />
        
        <Container className="position-relative" style={{ zIndex: 1, padding: '50px 15px' }}>
          <Row>
            <Col lg={8} md={10}>
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                <h1 style={{ fontWeight: 800, fontSize: '3.8rem', lineHeight: 1.2, color: '#fff', marginBottom: '15px' }}>
                  HIGH PRECISION <br /> ONTIME DELIVERY
                </h1>
                <div style={{ fontSize: '1.25rem', fontWeight: 600, color: '#fff', letterSpacing: '1px', marginBottom: '35px' }}>
                  AUTOMATED PRODUCTION TECHNOLOGY WITH ROBOTIC
                </div>
                <Link to="/capabilities" style={{ backgroundColor: '#009CD7', color: '#fff', textDecoration: 'none', padding: '12px 35px', borderRadius: '30px', fontWeight: 'bold', fontSize: '14px', display: 'inline-block' }}>
                  KNOW MORE
                </Link>
              </motion.div>
            </Col>
          </Row>
        </Container>
      </section>

      {/* We Are SDN Section */}
      <section style={{ backgroundColor: '#fff', padding: '60px 0' }}>
        <Container>
          <Row>
            <Col lg={8}>
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
                <div style={{ color: '#888', fontWeight: '700', fontSize: '0.95rem', marginBottom: '10px', letterSpacing: '1px' }}>WE ARE</div>
                <h2 style={{ color: '#009CD7', fontWeight: 'bold', fontSize: '2.8rem', marginBottom: '25px', lineHeight: '1.2' }}>
                  SDN ENGINEERING
                </h2>
                <h4 style={{ color: '#000', fontWeight: 'bold', fontSize: '1.1rem', lineHeight: '1.6', marginBottom: '20px' }}>
                  Manufacturer of Precision Machined Components for Automobile and General Engineering Industries
                </h4>
                <p style={{ color: '#555', fontSize: '0.95rem', lineHeight: '1.8', marginBottom: '0' }}>
                  With state-of-the-art CNC and VMC technology, SDN Engineering provides world-class quality and consistency across every part we produce.
                </p>
              </motion.div>
            </Col>
          </Row>
        </Container>
      </section>

      {/* About Section with Stats Card */}
      <section style={{ position: 'relative', backgroundColor: '#f9f9f9', padding: '60px 0' }}>
        <Container>
          <Row className="g-0" style={{ position: 'relative', alignItems: 'center' }}>
            {/* Right Column: Large Image */}
            <Col lg={7} style={{ position: 'relative' }}>
              <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
                <img 
                  src="/gallery/9.jpeg" 
                  alt="Industrial Equipment" 
                  style={{ width: '100%', height: '550px', objectFit: 'cover', display: 'block' }} 
                />
              </motion.div>
            </Col>

            {/* Left Column: Overlapping Stats Card */}
            <Col lg={5} style={{ position: 'relative', paddingLeft: '30px' }}>
              <motion.div 
                initial={{ opacity: 0, x: -50 }} 
                whileInView={{ opacity: 1, x: 0 }} 
                viewport={{ once: true }} 
                transition={{ duration: 0.8 }}
                style={{ 
                  backgroundColor: '#fff',
                  padding: '40px 30px',
                  boxShadow: '0 8px 25px rgba(0,0,0,0.12)',
                  position: 'relative',
                  zIndex: 10,
                  marginLeft: '-80px'
                }}
              >
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px 20px' }}>
                  {/* Stat 1 */}
                  <div style={{ borderRight: '1px solid #e0e0e0', paddingRight: '20px' }}>
                    <div style={{ fontSize: '0.9rem', color: '#000', fontWeight: '700', lineHeight: '1.5', textAlign: 'left' }}>
                      <span style={{ display: 'block', fontWeight: 'bold', marginBottom: '5px' }}>1 Million+</span>
                      precision machined components produced annually
                    </div>
                  </div>

                  {/* Stat 2 */}
                  <div style={{ paddingLeft: '0px' }}>
                    <div style={{ fontSize: '0.9rem', color: '#000', fontWeight: '700', lineHeight: '1.5', textAlign: 'left' }}>
                      <span style={{ display: 'block', fontWeight: 'bold', marginBottom: '5px' }}>70%</span>
                      current utilization — ready to scale production capacity and enhancements
                    </div>
                  </div>

                  {/* Stat 3 */}
                  <div style={{ borderRight: '1px solid #e0e0e0', paddingRight: '20px', borderTop: '1px solid #e0e0e0', paddingTop: '20px' }}>
                    <div style={{ fontSize: '0.9rem', color: '#000', fontWeight: '700', lineHeight: '1.5', textAlign: 'left' }}>
                      Complete end-to-end machining, assembly & inspection under one roof
                    </div>
                  </div>

                  {/* Stat 4 */}
                  <div style={{ borderTop: '1px solid #e0e0e0', paddingTop: '20px' }}>
                    <div style={{ fontSize: '0.9rem', color: '#000', fontWeight: '700', lineHeight: '1.5', textAlign: 'left' }}>
                      ISO 9001:2015 certified and IATF 16949 Certificate Audit Completed 31.July25
                    </div>
                  </div>
                </div>
              </motion.div>
            </Col>
          </Row>
        </Container>
      </section>

      {/* Why SDN Section */}
      <section className="py-5 position-relative" style={{ backgroundImage: "url('https://sdn.co.in/wp-content/uploads/2025/10/shutterstock_1492926674-1-2.png')", backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed', padding: '80px 0' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0, 156, 215, 0.85)', zIndex: 0 }}></div>
        
        <Container className="position-relative" style={{ zIndex: 1 }}>
          <div className="text-center mb-5">
            <h2 style={{ color: '#fff', fontSize: '2.5rem', fontWeight: 'bold', letterSpacing: '1px' }}>WHY SDN</h2>
          </div>
            
          <Row className="g-4">
            <Col lg={3} md={6}>
              <div className="text-center">
                <img src="https://sdn.co.in/wp-content/uploads/2025/10/Asset-1sapi-150x150-1.png" style={{ width: '80px', marginBottom: '15px' }} alt="Quality Icon" />
                <h3 style={{ color: '#fff', fontSize: '1.2rem', fontWeight: 'bold', marginBottom: '15px' }}>Quality</h3>
                <p style={{ color: '#fff', fontSize: '0.9rem', lineHeight: '1.6' }}>
                  We are an ISO 9001:2015 certified organization, committed to delivering precision-machined components that meet stringent customer specifications. Our robust Quality Management System ensures consistency, traceability, and "zero-defect" products through continuous monitoring and process improvement.
                </p>
              </div>
            </Col>
            <Col lg={3} md={6}>
              <div className="text-center">
                <img src="https://sdn.co.in/wp-content/uploads/2025/10/Asset-2sapi-150x150-1.png" style={{ width: '80px', marginBottom: '15px' }} alt="Timely Delivery Icon" />
                <h3 style={{ color: '#fff', fontSize: '1.2rem', fontWeight: 'bold', marginBottom: '15px' }}>Timely Delivery</h3>
                <p style={{ color: '#fff', fontSize: '0.9rem', lineHeight: '1.6' }}>
                  With optimized production planning and a customer-specific scheduling system, SDN ensures on-time delivery of every batch. Our efficient supply chain and strong vendor network enable us to meet tight timelines without compromising quality.
                </p>
              </div>
            </Col>
            <Col lg={3} md={6}>
               <div className="text-center">
                <img src="https://sdn.co.in/wp-content/uploads/2025/10/Asset-4sapi-150x150-1.png" style={{ width: '80px', marginBottom: '15px' }} alt="Precision Icon" />
                <h3 style={{ color: '#fff', fontSize: '1.2rem', fontWeight: 'bold', marginBottom: '15px' }}>Precision Manufacturing</h3>
                <p style={{ color: '#fff', fontSize: '0.9rem', lineHeight: '1.6' }}>
                  At SDN Engineering, precision is our foundation. Equipped with advanced CNC turning centers, VMCs, thread rolling machines, and robotic automation, we manufacture critical components with close tolerances and superior finish — ensuring performance, accuracy, and reliability in every part.
                </p>
              </div>
            </Col>
            <Col lg={3} md={6}>
               <div className="text-center">
                 <img src="https://sdn.co.in/wp-content/uploads/2025/10/Asset-3sapi-150x150-1.png" style={{ width: '80px', marginBottom: '15px' }} alt="Customer Focus Icon" />
                 <h3 style={{ color: '#fff', fontSize: '1.2rem', fontWeight: 'bold', marginBottom: '15px' }}>Customer Focus</h3>
                 <p style={{ color: '#fff', fontSize: '0.9rem', lineHeight: '1.6' }}>
                   We are a customer-driven company that believes in building long-term partnerships. By working closely with our clients from concept to production, we develop the best solutions to meet their functional, quality, and delivery requirements — every time.
                 </p>
              </div>
            </Col>
          </Row>
        </Container>
      </section>

      {/* Pre-footer Gear Image Section */}
      <div style={{ height: '350px', backgroundImage: "url('https://sdn.co.in/wp-content/uploads/2025/10/shutterstock_1950663952-1-1.png')", backgroundSize: 'cover', backgroundPosition: 'center' }}>
      </div>
    </div>
  );
};

export default Home;
